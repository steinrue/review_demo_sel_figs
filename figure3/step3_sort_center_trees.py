...
def find_median_from_reps(cond, heightfile, treeSkeleton, replist, sample_size, scaling_factor, seed=None):
    '''Randomly sample <sample_size> lineages, extract the center tree for each rep in <replist>,
     record their heights, write them to file & return the list of heights'''
    # set seed
    if seed is None:
...
def plot_center_tree(samp_trees, sigma: float, sel_loc=None):
    ...
    # hide_leaf_symlabs = ".node.leaf > .sym, .node.leaf > .lab {display: none}" #+ hide_leaf_symlabs
    color_branches = ".mut.m2 .sym, .m2>line, .m2>.node .edge{stroke:darkred} .mut.m2 .lab{fill:red}"
    css_string = hide_internal_symlabs + color_branches

    svgstr = center_tree.draw_svg(size=(400, 650),
...
def main():
    ...
    heightfile = f'precomputed/TreeHeights_{cond}_rep{from_rep}-{to_rep}_n{sample_size}_seed{seed}_lambda20.tsv'

    if os.path.exists(heightfile):
        height_list = pd.read_csv(heightfile, sep="\t")
        Tmrca_list = np.array(height_list['mean_root_age'])
        med_idx, med_height, med_seed, med_num_roots, med_batch_Hs = height_list.iloc[numReps // 2 + 1,]
    else:
        ...
        Tmrca_list = np.array([height[1] for height in height_list])
